...
export async function mute(_, qq, time, groupNumber){
    ...
    if (time == "0") {
      _.session.send(
        `在群聊 ${groupInfo.group_name}(${groupNumber}) 中解除了 ${groupMemberInfo.nickname}(${qq}) 的禁言。`)
    }
    else {
      _.session.send(
        `在群聊 ${groupInfo.group_name}(${groupNumber}) 中禁言了 ${groupMemberInfo.nickname}(${qq}) ${Math.floor(parseInt(time) / 60)} 分钟。`)
    }
}

export async function allmute(_, tf, groupNumber){
    if (groupNumber == undefined) { groupNumber = _.session.guildId }
