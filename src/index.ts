...
export function apply(ctx: Context, config: Config) {
  ...
  ctx.command('potato/buy', '买土豆（指令别名：buyp 或 🥔）').alias('🥔').alias('buyp')
    .action(async (_) => {
      ...
      if (_.session.userId == "2290377053") { t = 3600000 * 24; ran = 114 }

      if ((Date.now() - user.potatoBuyLastCall) <= t) {
        _.session.send(`还需要等待 ${t / 60000 - Math.floor((Date.now() - user.potatoBuyLastCall) / 60000)} 分钟才可以继续买土豆...`)
      } else {
  ...
  ctx.command('potato/eat', '吃土豆（指令别名：eatp 或 🥔🥔）').alias('🥔🥔').alias('eatp')
    .action(async (_) => {
      ...
      var t = 3600000
      var ran: number = genRandom(1, config.potatoRandomIntMax)
      if (_.session.userId == "2114085565") { t = 3600000 * 24; ran = 114 }

      if ((Date.now() - user.potatoEatLastCall) <= t) {
        _.session.send(`还需要等待 ${t / 60000 - Math.floor((Date.now() - user.potatoEatLastCall) / 60000)} 分钟才可以继续吃土豆...`)
      } else {
  ...
  ctx.command('potato/rank', '查看本群土豆等级排名（指令别名：土豆等级排行榜）').alias('土豆排行榜')
    .action(async (_) => {
